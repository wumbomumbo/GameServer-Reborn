...
router.get("/pids/:who/personas", async (req, res, next) => { // EA Accounts
  try {
    ...
    const USER_BY_UID_QUERY = "SELECT UserId, UserAccessToken FROM UserData WHERE UserId = ?;";
    await db.get(USER_BY_UID_QUERY, [req.params.who], async (error, row) => {
	    if (!row) {
		res.status(400).send({"message": "No user could be found with that UserId"});
		return;
	    }
...
router.get("/pids/me/personas/:who", async (req, res, next) => { // Anonymous Accounts
  try {
    const token = req.headers["authorization"].split(" ")[1];
    if (!token) {
	    res.status(400).send({"message": "No Authorization Header"});
