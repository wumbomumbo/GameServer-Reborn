...
router.get("/auth", async (req, res, next) => {
  try {
    ...
    if (req.query.authenticator_login_type == "mobile_anonymous") { // Generate new account
	    const LAST_USER_QUERY = "SELECT MayhemId, UserId from UserData ORDER BY UserId DESC LIMIT 1;"; 
	    await db.get(LAST_USER_QUERY, async (error, row) => { // Get the last created user, so we can make the new users ids +1
		    if (error) {
		    ...
		    const newUID = ((row) ? Number(row.UserId) + 1 : config.startingUID + 1 /* If there are no users */ );
		    const newMID = ((row) ? Number(row.MayhemId) + 1 : config.startingMID + 1 /* If there are no users */ );
...
router.post("/token", async (req, res, next) => {
  try {
    ...
    const USER_BY_CODE_QUERY = "SELECT UserId, UserAccessToken FROM UserData WHERE UserAccessCode = ?;";
...
router.get("/tokeninfo", async (req, res, next) => {
  try {
    ...
    const USER_BY_TOKEN_QUERY = "SELECT UserId FROM UserData WHERE UserAccessToken = ?;";
    await db.get(USER_BY_TOKEN_QUERY, [accessToken], async (error, row) => {
	    ...
	    let response =  {
		    ...
		    "scope": "offline basic.antelope.links.bulk openid signin antelope-rtm-readwrite search.identity basic.antelope basic.identity basic.persona antelope-inbox-readwrite", // Always this for anonymous accounts
		    "user_id": row.UserId.toString()
	    };
